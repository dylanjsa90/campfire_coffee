...
function renderRow(id, name, locationTotal, totalArray) {
  ...
  tdElement.textContent = Math.ceil(locationTotal);
  trElement.appendChild(tdElement);
  total += Math.ceil(locationTotal);
  for (var i = 0; i < hours.length; i++) {
    tdElement = document.createElement('td');
    tdElement.textContent = Math.ceil(totalArray[i]);
    totals[i] += Math.ceil(totalArray[i]);
    trElement.appendChild(tdElement);
  }
...
function handleLocationSubmit(event) {
  ...
  for (var i = 0; i < coffeeLocations.length; i++) {
    if (locationName === coffeeLocations[i].locationName) {
      ...
      coffeeLocations[i] = newLocationData;
...
function renderAll() {
  renderTable('beans-table', 'Daily Location ', 'totalBeans', 'totalBeansPerHour');
  renderTable('baristas-table', '', 'totalBarristaHours', 'barristaPerHour');
}

function renderTable(id, heading,total, totalArray) {
  tableHeadings(heading, id);
  for (var i = 0; i < coffeeLocations.length; i++) {
    calculateData(coffeeLocations[i]); // Fill instance data for all locations
    renderRow(id, coffeeLocations[i].locationName, coffeeLocations[i][total], coffeeLocations[i][totalArray]); // Fill in all table rows
  }
  totalRow(id);
}
